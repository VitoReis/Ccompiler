...
def verify(token, file, s):
    global lastImportant, logicalOperators, arithmetichOperators, cbList

    if token[1] == 'ID':
        nexToken = file[s].split('~')
        if lastImportant == 'INT' or lastImportant == 'FLOAT' or lastImportant == 'CHAR':
